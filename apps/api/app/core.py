"""Shared application services used by every domain router.

Formerly the body of main.py. Routers import from here (``from ..core import``)
instead of from main.py, so importing a router no longer imports the FastAPI
app and its router registration (which caused circular-import failures when a
router module was imported on its own). main.py re-exports these names for
