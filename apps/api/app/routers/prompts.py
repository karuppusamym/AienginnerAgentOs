"""Auto-extracted prompts routes from the former monolithic main.py.
...
@router.post("/prompts/{prompt_id}/rollback", status_code=201)
...
def delete_prompt(prompt_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict[str, str]:
    ...
    if prompt is None or prompt.project_id != project.id or prompt.artifact_type != "prompt":
        raise HTTPException(status_code=404, detail="Prompt not found")
    db.execute(delete(ArtifactComment).where(ArtifactComment.artifact_id == prompt.id))
    db.execute(delete(ArtifactVersion).where(ArtifactVersion.artifact_id == prompt.id))
    db.delete(prompt)
    audit(db, user, "prompt.deleted", "artifact", prompt_id)
    db.commit()
