"""Auto-extracted auth routes from the former monolithic main.py.
...
from ..auth import SESSION_COOKIE, TOKEN_TTL_HOURS
...
LOGIN_FAILURES_PER_WINDOW = int(os.getenv("LOGIN_FAILURES_PER_WINDOW", "10"))


@router.post("/auth/login")
def login(payload: LoginRequest, request: Request, response: Response, db: Session = Depends(get_db)) -> dict[str, Any]:
    client_ip = request.client.host if request.client else "unknown"
    throttle_key = f"login:{payload.email.lower()}:{client_ip}"
    user = db.scalar(select(User).where(func.lower(User.email) == payload.email.lower()))
    if user is None or not user.active or not verify_password(payload.password, user.password_hash):
        # Only failures count, so a legitimate user is never throttled by their own sign-ins.
        limited = check_rate_limit(throttle_key, LOGIN_FAILURES_PER_WINDOW, 300, local_fallback=True)
        if not limited.allowed:
            raise HTTPException(status_code=429, detail="Too many failed sign-in attempts; try again in a few minutes", headers={"Retry-After": str(limited.retry_after_seconds)})
...
@router.put("/auth-providers/{provider_id}")
