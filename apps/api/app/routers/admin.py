"""Auto-extracted admin routes from the former monolithic main.py.
...
def create_user(
    ...
    if default_project is not None:
        # Project roles are authoritative inside a project, so derive the
        # starting project role from the global role (viewer stays read-only).
        membership_role = default_membership_role(payload.role)
        db.add(ProjectMembership(project_id=default_project.id, user_id=user.id, role=membership_role, is_current=True))
