...
4. [Architecture decisions](ARCHITECTURE_DECISIONS.md) - graph database / lineage-traversal recommendation, the agentic harness and self-learning approach, and UI-accessibility direction.
   ...
   - [Learning loop](LEARNING_LOOP.md) - how GEPA prompt optimisation, multi-model SQL voting, verified-query memory, slow-query DDL suggestions and Jev routing work, and what was measured.
   - [Agents, tools and Jev](AGENTS_TOOLS_AND_JEV.md) - internal vs external agents and tools, which decisions Jev makes (route, agent, risk, tool per step, SQL tie-break), the live demo script and its latest proof report, and the prioritised improvement list.
   - [Embedded analytics](ANALYTICS_EMBEDDING.md) - the dashboard picker (project dashboard, published queries, datasets by source), why scope is the project, and the governance on each dashboard.
